...
.. envvar:: ID_TRANSLATION_DISABLED
   ...
   Global switch. When ``true``, the :meth:`Translator.translate <id_translation.Translator.translate>`-method returns
   immediately.
...
.. envvar:: ID_TRANSLATION_SUPPRESS_OPTIONAL_FETCHER_INIT_ERRORS
   ...
   discard `optional` fetchers that raise when imported or initialized. See the :ref:`documentation <optional-fetchers>`
   for details.

.. envvar:: ID_TRANSLATION_VERBOSE

   Global switch. Set to ``true`` to overwrite the default :data:`~id_translation.logging.ENABLE_VERBOSE_LOGGING` value.
   Note that this variable is only read once (on module import). See the :ref:`documentation <translation-logging>` for
   details.
"""
