...
class Mapper(Generic[ValueType, CandidateType, ContextType]):  # noqa: PLW1641
    ...
    Args:
        ...
        overrides: If a dict, assumed to be 1:1 mappings (`value` to `candidate`) which override the scoring logic. If
            :class:`rics.collections.dicts.InheritedKeysDict`, the context passed to
            :meth:`~id_translation.mapping.Mapper.apply` is used to retrieve specific overrides.
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        Returns:
            A :class:`~id_translation.mapping.DirectionalMapping` on the form ``{value: [matched_candidates..]}``. May
            be turned into a plain dict ``{value: candidate}`` by using the
            :meth:`DirectionalMapping.flatten <id_translation.mapping.DirectionalMapping.flatten>` function (only if
    ...
    def copy(self, **overrides: Any) -> Self:
        ...
        Args:
            overrides: Keyword arguments to use when instantiating the copy. Options that aren't given will be taken
                from the current instance. See the :class:`~id_translation.mapping.Mapper` class documentation for
                possible choices.
