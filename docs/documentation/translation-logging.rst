...
Key event locations
-------------------
Key event records are emitted on the ``INFO`` and ``DEBUG`` level. These are events emitted at important points that
output structured data which may be ingested by common log ingestion tools (more on that later).
...
There may be intermediate ``event_stage`` keys emitted, however this will not be discussed in this document. Instead,
let's jump right into an example.

Sample record
-------------
Below is the final success message produced by a single :meth:`.Translator.translate`-call with every single debug
option enabled, about 400 records in total. Only 2 of these are non-``DEBUG`` messages; the one at the bottom and the
