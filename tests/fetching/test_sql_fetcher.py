...
def test_bad_override(column, connection_string):
    ...
    with pytest.raises(exceptions.UnknownPlaceholderError, match=repr(column)):
        fetcher.fetch([IdsToFetch("humans", {-1})], ("id", "name"), required=("id", "name"))
    fetcher.close()
...
def test_disconnected(connection_string):
    ...
    with (
        pytest.warns(FutureWarning, match=r"force=\.\.\."),
        pytest.raises(ConnectionStatusError, match=r"disconnected: 'sqlite"),
    ):
